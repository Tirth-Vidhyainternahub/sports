...
const Homepage = (req, res) => {
  res.render("index", {
    title: "Dashboard",
    description: "Welcome to the Sports Dashboard",
    keywords: "sports, dashboard, admin",
    ...
    cards: [
      {
        ...
        title: "Audience",
        count: 0,
        icon: `<svg version="1.1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px" viewBox="0 0 512.126 512.126" xml:space="preserve">
...
const LoginPage = (req, res) => {
  res.render("login", {
    title: "Login - Score Tab",
    description: "Login to Score Tab dashboard",
    keywords: "score tab, login, dashboard",
    author: "Score Tab",
  });
};
...
const CityPage = async (req, res) => {
  try {
    const citydata = await City.find().populate("country");
...
const venuePage = async (req, res) => {
  try {
    const venuedata = await Venue.find().populate("city");
...
const sportPage = async (req, res) => {
  try {
    const sportdata = await Sport.find();
    const sportsCategoriesdata = await SportCategory.find();
    res.render("sports", {
      ...
      categories: sportsCategoriesdata?.data?.sportsCategories || [],
    });
  } catch (err) {
    errorHandler(res, 500, "Error while fetching data", error);
  }
...
const ottPage = async (req, res) => {
  try {
    const ottdata = await OTT.find();
...
const telecastPage = async (req, res) => {
  try {
    const telecastdata = await Telecast.find().populate("ott");
    const countriesdata = await Country.find();
    res.render("telecast", {
      title: "Telecast Management",
      user: {
        name: name, // or get from session/database
        role: role, // or actual user role
      },
      telecasts: telecastdata?.data?.telecasts || [],
      countries: countriesdata?.data?.countries || [],
    });
  } catch (err) {
    errorHandler(res, 500, "Error while fetching data", error);
...
const playerPage = async (req, res) => {
  try {
    const playerdata = await Player.find().populate("team");
    const sportdata = await Sport.find();
    const countriesdata = await Country.find();
    res.render("player", {
      ...
      players: playerdata?.data?.players || [],
      sports: sportdata?.data?.sports || [],
      countries: countriesdata?.data?.countries || [],
    });
  } catch (err) {
...
const teamPage = async (req, res) => {
  try {
    const teamdata = await Team.find().populate("sport");
    const sportdata = await Sport.find();
    const countriesdata = await Country.find();
    res.render("team", {
